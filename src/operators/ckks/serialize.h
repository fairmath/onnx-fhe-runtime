...
struct CustomOpCKKSLoader : Ort::CustomOpBase<CustomOpCKKSLoader, CKKSLoaderKernel> {
    explicit CustomOpCKKSLoader(std::shared_ptr<reg::CryptoRegistry> registry)
        : registry_(registry) {}

    void* CreateKernel(OrtApi api, const OrtKernelInfo* info) const {
        return new CKKSLoaderKernel(api, info, registry_);
    }
    const char* GetName() const { return "fhe.ckks.loader"; }
...
struct CustomOpCKKSSaver : Ort::CustomOpBase<CustomOpCKKSSaver, CKKSSaverKernel> {
    explicit CustomOpCKKSSaver(std::shared_ptr<reg::CryptoRegistry> registry)
        : registry_(registry) {}

    void* CreateKernel(OrtApi api, const OrtKernelInfo* info) const {
        return new CKKSSaverKernel(api, info, registry_);
    }
    const char* GetName() const { return "fhe.ckks.saver"; }
    ONNXTensorElementDataType GetInputType(size_t index) const {
        return ONNX_TENSOR_ELEMENT_DATA_TYPE_STRING;
    }
